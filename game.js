...
function buildB2Grid() {
  ...
  grid.innerHTML = '';
  (window.CHARACTERS || []).forEach((char, i) => {
...
function openGallery() {
  // Guard: wait for async card loader if not ready yet
  if (!window.CARDS || !window._NCG_READY) {
    if (!document.getElementById('_gallery-loading')) {
      const loadMsg = document.createElement('div');
      loadMsg.id = '_gallery-loading';
      loadMsg.style.cssText = 'position:fixed;inset:0;z-index:200;display:flex;align-items:center;justify-content:center;background:rgba(5,8,16,0.85);font-family:var(--font-ui);font-size:14px;color:var(--accent-teal);letter-spacing:.1em;';
      loadMsg.textContent = 'Loading cards…';
      document.body.appendChild(loadMsg);
    }
    document.addEventListener('ncg:ready', function _onReady() {
      const el = document.getElementById('_gallery-loading');
      if (el) el.remove();
      openGallery();
    }, { once: true });
    return;
...
function _runInit() {
  ...
  updateFloatingMenu();
  buildB2Grid();
  computeCardTextStats();
  registerBuffsFromData();
}

_runInit();

// If loader.js hasn't finished yet, re-run init once cards are ready
// so the character grid, gallery, etc. all populate correctly
if (!window._NCG_READY) {
  document.addEventListener('ncg:ready', _runInit, { once: true });
}
...
function buildDevScreen() {
  ...
  // Only mark built AFTER confirming DOM elements exist
  _devBuilt = true;
