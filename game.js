...
function buildB2Grid() {
  ...
  CHARACTERS.forEach((char, i) => {
    ...
    box.addEventListener('click', () => startBattle(i));
...
/* B4-1: Enemy deck preview — called when player clicks either portrait on Dream Ready.
   Builds the same random enemy pool used at battle start so the preview is representative.
   Both char and dazed portraits show the same shared pool; only their skill cards differ. */
function drShowEnemyDeckPreview(side) {
  ...
  const nonEv   = CARDS.filter(c => c.type !== 'EV' && !c.isDefault);
  const pool    = shuffleCopy([...nonEv]).slice(0, 50);
  const preview = [...pool, ...evCards.slice(0, 3)];
...
function initGameState() {
  ...
  // Build enemy deck: pull event cards from their pool (tagged to this character),
  // fill the rest with cards from CARDS pool minus default cards
  const evCards  = CARDS.filter(c => c.type === 'EV' && (!c.charTag || c.charTag === char.cardId));
  const nonEv    = CARDS.filter(c => c.type !== 'EV' && !c.isDefault);
  const enemyPool = shuffleCopy([...nonEv]);
  // Build enemy 50-card deck: up to 50 from pool
  const enemyDeckIds = enemyPool.slice(0, 50).map(c => c.id);

  // Pick 3 random event cards for event selection
  ...
  // Determine enemy main char
  const enemyCharCards = CARDS.filter(c => ['CH','DZ','DA'].includes(c.type));
...
function drawCard(who, animate = true) {
  ...
  if (GS[deckKey].length === 0) {
    ...
    GS[discardKey] = [];
    updatePileCounts();
    if (who === 'player') showToast('Deck reshuffled from discard pile!');
  }
...
function toggleDrawer() {
  ...
  tab.textContent = `${DRAWER_OPEN ? '▼' : '▲'} Hand (`;
...
function renderPlayerHand() {
  ...
  GS.playerHand.forEach((cardId, idx) => {
    const card = CARDS.find(c => c.id === cardId);
    if (!card) return;
...
/* Skill card type → parent type mapping */
};
...
function evaluateCondition(condStr, card, inst, who, effectiveRank) {
  ...
  if (lower === 'canbanish'  || lower === 'canbanish:true')     return (inst?.canBanish !== false);
  ...
  if (lower.startsWith('canheal'))       return inst?.canHeal       !== false;
...
function _doAction(tok, key, card, inst, who, effectiveRank) {
  ...
  // ── CanTakeDamage / CanHeal / CanBeBuffed / CanBeDebuffed ──
  if (key === 'cantakedamage')  { if (inst) inst.canTakeDamage  = (tok.val !== 'false'); return; }
  if (key === 'canheal')        { if (inst) inst.canHeal        = (tok.val !== 'false'); return; }
...
/* =============================================
   ...
   Runtime instance (GS.field[r][c]) shape:
   {
     ...
     canCardDestroy, canBanish, canRevive,
     canTakeDamage, canHeal,
     canBeBuffed, canBeDebuffed,
...
function resolveTargets(targetStr, contextCard, contextInst, who) {
  ...
  // TargetedCard — the card currently targeted by player/enemy action
  ...
  // LastCard — last card interacted with (stored in GS.lastPickedInst)
  if (ts.startsWith('LastCard')) {
    if (GS.lastPickedInst) {
      const card = CARDS.find(c => c.id === GS.lastPickedInst.cardId);
      return card ? [{ card, inst: GS.lastPickedInst }] : [];
    }
    return [];
  }
...
function banishCard(inst, who) {
  if (!inst || inst.canBanish === false) return;
...
function renderBuffDebuffIcons(inst, container) {
  ...
  ['appliedBuffs','appliedDebuffs'].forEach(key => {
    ...
    arr.forEach(entry => {
      ...
      // Short label (first 2 chars of id)
...
function renderFieldCell(row, col) {
  ...
  fc.className = 'gf-field-card' + (data.owner === 'enemy' ? ' enemy-card' : '');
...
function resolveAttack(srcR, srcC, targetR, targetC, isPortrait) {
  ...
  if (isPortrait) {
    // Attack enemy portrait — route through B4-4 HP tracking
...
function isPlayerTurn() {
  ...
  const playerTurns = GS.whoFirst === 'player' ? [1,3,5,7,9,11] : [2,4,6,8,10,12];
  return playerTurns.includes(((GS.turn - 1) % 12) + 1) ||
    (GS.whoFirst === 'player' ? GS.turn % 2 === 1 : GS.turn % 2 === 0);
}
...
function destroyFieldCard(r, c, owner) {
  ...
  // Get the DOM element for this cell so the animation starts from the right place
...
function endBattle() {
  ...
  GS.playerEnergy  = 10;
  GS.enemyEnergy   = 10;
  GS.playerBonusEnergy = 0;
  GS.enemyBonusEnergy  = 0;
  GS.actionLog         = [];
  GS.cardOrderLog      = [];
  GS.cardPlayLog       = [];
  GS.lastEffectiveRank = 1;
...
function triggerSecondChance() {
  ...
  // Load dazed art from the character being fought
  const char = CHARACTERS.find(c => c.id === GS.selectedCharId);
  const dazedCard = char ? CARDS.find(c => c.id === char.dazedCardId) : null;
  if (dazedCard?.img) {
    img.src = dazedCard.img;
    img.style.display = 'block';
    ph.style.display  = 'none';
  } else {
    img.style.display = 'none';
    ph.textContent    = dazedCard?.name || 'DAZED';
...
function confirmSecondChance() {
  ...
  setTimeout(() => {
    ...
    // Reset overlay internals for potential reuse
  ...
  GS.secondChance = true;
  GS.gameOver     = false;  // allow game to continue

  // ── Card types that are STRIPPED from player (B4-4-C spec) ───────
  const STRIPPED_TYPES = new Set(['CHS','CHSS','DZS','DZSS','DA','DAS','DASS','DR','EV','DZ']);

  // Helper: is a card id strippable?
  ...
  for (let row = 0; row < 2; row++) {
    for (let col = 0; col < 5; col++) {
      const cellId = `gf-cell-enemy-${row}-${col}`;
      const cell   = document.getElementById(cellId);
      if (cell && cell.dataset.cardId) {
        const cid = cell.dataset.cardId;
        GS.enemyDiscard = GS.enemyDiscard || [];
        GS.enemyDiscard.push(cid);
        cell.innerHTML    = '';
        cell.dataset.cardId = '';
        cell.classList.remove('occupied');
        // Visual: flash cell briefly
        cell.style.background = 'rgba(224,60,90,0.25)';
  ...
  GS.activeEventCard = null;
  const evSlot = document.getElementById('gf-event-slot');
  if (evSlot) {
    evSlot.innerHTML = '';
    evSlot.style.background = 'rgba(224,60,90,0.1)';
    setTimeout(() => { evSlot.style.background = ''; }, 600);
  }

  // ── 3. Destroy player's Dozer cards on field ──────────────────────
  for (let row = 2; row < 4; row++) {
    for (let col = 0; col < 5; col++) {
      const cellId = `gf-cell-player-${row - 2}-${col}`;
      const cell   = document.getElementById(cellId);
      if (cell && cell.dataset.cardId) {
        const card = CARDS.find(c => c.id === cell.dataset.cardId);
        if (card && STRIPPED_TYPES.has(card.type)) {
          GS.playerDiscard = GS.playerDiscard || [];
          GS.playerDiscard.push(cell.dataset.cardId);
          cell.innerHTML      = '';
          cell.dataset.cardId = '';
          cell.classList.remove('occupied');
          cell.style.background = 'rgba(224,60,90,0.25)';
  ...
  // ── 5. Strip player deck ──────────────────────────────────────────
  const strippedFromDeck = (GS.playerDeck || []).filter(id => isStripped(id));
  GS.playerDeck          = (GS.playerDeck || []).filter(id => !isStripped(id));
  // Stripped deck cards go nowhere (removed from game per spec)

  // ── 6. Strip enemy deck/hand of Dreamscape cards ─────────────────
  const ENEMY_STRIPPED = new Set(['DR']);
  GS.enemyHand = (GS.enemyHand || []).filter(id => {
    const card = CARDS.find(c => c.id === id);
    return card ? !ENEMY_STRIPPED.has(card.type) : true;
  });
  GS.enemyDeck = (GS.enemyDeck || []).filter(id => {
    const card = CARDS.find(c => c.id === id);
    return card ? !ENEMY_STRIPPED.has(card.type) : true;
  });

  // ── 7. Reshuffle both discard piles into new decks ────────────────
  //   Only include cards NOT stripped (per spec: exclude deleted cards)
  function shuffleArray(arr) {
    for (let i = arr.length - 1; i > 0; i--) {
      const j = Math.floor(Math.random() * (i + 1));
      [arr[i], arr[j]] = [arr[j], arr[i]];
  ...
  GS.playerDeck    = shuffleArray([...GS.playerDeck, ...playerDiscardClean]);
  GS.playerDiscard = [];

  const enemyDiscardClean = (GS.enemyDiscard || []).filter(id => {
    const card = CARDS.find(c => c.id === id);
    return card ? !ENEMY_STRIPPED.has(card.type) : true;
  });
  GS.enemyDeck    = shuffleArray([...GS.enemyDeck, ...enemyDiscardClean]);
  GS.enemyDiscard = [];

  // ── 8. Reset turn counter; player always goes first ───────────────
  GS.turnNumber   = 1;
  GS.currentTurn  = 'player';
  GS.firstTurnDone = false;  // reset so no card draw on very first SC turn
  ...
  if (enemyPortrait) {
    ...
    if (dazedCard?.img) {
      enemyPortrait.style.backgroundImage = `url('${dazedCard.img}')`;
    }
  }

  // ── 11. Reset enemy main HP to Dazed card stats ───────────────────
  if (dazedCard && GS.mainHp) {
    GS.mainHp.enemy = {
      hp:     dazedCard.hp  ?? 20,
      shd:    dazedCard.shd ?? 0,
      maxHp:  dazedCard.hp  ?? 20,
      maxShd: dazedCard.shd ?? 0,
    };
    GS.enemyMainCard = dazedCard.id;
  }
  ...
  logAction('Skill, Sub-Skill, Dreamscape & Dozer cards stripped.', 'system');
  logAction('Location cards remain. Player goes first.', 'system');

  // ── 14. Win/lose hooks now route to correct result screens ────────
  //   checkWinCondition() already handles secondChance state:
  //     enemy HP → 0  ⟹ showResultScreen('dazed_defeated')
  //     player HP → 0 ⟹ showResultScreen('game_over')
  //   (secondChance flag blocks re-triggering triggerSecondChance)
...
/* ── helper: clear & re-render the action log for SC fresh start ── */
function clearActionLog() {
  const log = document.getElementById('gf-action-log');
  if (log) log.innerHTML = '';
  GS.actionLog = [];
}

/* ── helper: update both energy bars (player + enemy) ── */
function updateEnergyBars() {
  // Player
  const pe = document.getElementById('gf-player-energy');
  const pb = document.getElementById('gf-player-bonus-energy');
  if (pe) {
    const segs = pe.querySelectorAll('.energy-seg');
    segs.forEach((s, i) => s.classList.toggle('filled', i < (GS.playerEnergy || 0)));
  }
  if (pb) pb.textContent = GS.playerBonusEnergy || 0;

  // Enemy
  const ee = document.getElementById('gf-enemy-energy');
  const eb = document.getElementById('gf-enemy-bonus-energy');
  if (ee) {
    const segs = ee.querySelectorAll('.energy-seg');
    segs.forEach((s, i) => s.classList.toggle('filled', i < (GS.enemyEnergy || 0)));
  }
  if (eb) eb.textContent = GS.enemyBonusEnergy || 0;
}

/* ── helper: update deck/discard count badges ── */
function updateDeckCounts() {
  const pdEl = document.getElementById('gf-player-deck-count');
  const edEl = document.getElementById('gf-enemy-deck-count');
  if (pdEl) pdEl.textContent = (GS.playerDeck || []).length;
  if (edEl) edEl.textContent = (GS.enemyDeck  || []).length;
}
...
document.querySelector('[onclick="goTo(\'screen-gallery\')"]').onclick = openGallery;
