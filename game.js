...
function evaluateCondition(condStr, card, inst, who, effectiveRank) {
  ...
  if (lower.startsWith('canrevive')) {
    const wantTrue = !lower.includes(':false');
    return wantTrue ? (inst?.canRevive !== false) : (inst?.canRevive === false);
  }
...
function _doAction(tok, key, card, inst, who, effectiveRank) {
  ...
  // ── C3-1-G: CanRevive — prevent a specific card from ever being revived ──
  // canrevive:false  → this card cannot be revived from the discard/banished pile
  // canrevive:true   → restores revivability (default)
  if (key === 'canrevive') {
    if (inst) inst.canRevive = (tok.val !== 'false');
    return;
  }
...
function applyStatChange(statLabel, tok, card, inst, who, effectiveRank) {
  ...
  if (!isMax && key === 'hp' && tok.op === '=' && String(tok.val).toLowerCase() === 'max:hp') {
    if (inst) {
      ...
      if (inst.canHeal !== false) applyCardEffect(card, 'when_healed', who);
  ...
  if (key === 'hp' && !isMax && delta > 0 && tok.op && (tok.op === '+' || tok.op === '=+') && inst?.canHeal !== false) {
...
function confirmSecondChance() {
  ...
  GS.enemyDeck    = shuffleCopy([...GS.enemyDeck, ...enemyDiscardClean]);
  GS.enemyDiscard = [];
...
const DEV_DATA = [
  ...
  // ── C3-1-G: Game Mechanics ────────────────────────────────────────────
  {
    ...
    vars: [
      ...
      { name:'banished',                  type:'action',   desc:'Removes a card from the game entirely. Goes to invisible banish pile.',                            example:'banished@ref:TargetedCard' },
      { name:'CanBanish:true/false',      type:'bool',     desc:'When false, this card cannot be banished.',                                                        example:'CanBanish:false' },
      { name:'revived:discarded/banished@ref', type:'action', desc:'Revives a card from the discard or banish pile to hand.',                                      example:'revived:discarded@ref:IsPlayer:InDiscardPile' },
      { name:'CanRevive:true/false',      type:'bool',     desc:'When false, this card cannot be revived from discard or banish.',                                  example:'CanRevive:false' },
      { name:'CanTakeDamage:true/false',  type:'bool',     desc:'When false, HP cannot be reduced and all incoming damage is zero.',                                example:'CanTakeDamage:false' },
      { name:'CanHeal:true/false',        type:'bool',     desc:'When false, HP cannot be restored by healing effects (max HP changes still work).',                example:'CanHeal:false' },
